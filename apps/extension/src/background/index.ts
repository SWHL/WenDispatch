...
import { db, type Job } from '@wendispatch/core';
...
import {
  ...
  markJobResultPublished as markManagedJobResultPublished,
  processScheduledJobs as processManagedScheduledJobs,
  retryJobTarget as retryManagedJobTarget,
...
async function handleMessage(message: any, sender: chrome.runtime.MessageSender) {
  ...
  switch (message.type) {
    ...
    case 'OPEN_JOB_TARGET': {
      const job = await db.jobs.get(message.data.jobId);
      const result = job?.results?.[message.data.resultIndex];
      if (!job || !result) throw new Error('发布结果不存在');
      const reuseKey = `${job.id}:${result.platform}:${result.accountId}`;
      const existing = await getReuseTabInfo(reuseKey);
      const url = existing?.url || result.url;
      if (!url) throw new Error('没有可打开的发布页面');
      await openOrReuseTab(url, { active: true, reuseKey, addToSyncGroup: true });
      return { success: true, url };
    }

    case 'MARK_JOB_RESULT_PUBLISHED':
      return await markManagedJobResultPublished(message.data.jobId, message.data.resultIndex);

    case 'RETRY_JOB_TARGET':
      return await retryManagedJobTarget(message.data.jobId, message.data.resultIndex);
