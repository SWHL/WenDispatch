...
**Architecture:** Put provider, prompt, parsing, and error logic in a new `@wendispatch/ai` package. Keep extension-specific storage, host permission, and message routing in a thin AI layer under `apps/extension/src/background` and `apps/extension/src/ui/options/ai`. Add only route/nav registration and one edit-entry decision to the existing options UI.
...
- `tsconfig.json`: add path alias for `@wendispatch/ai`.
- `apps/extension/package.json`: add workspace dependency on `@wendispatch/ai`.
- `apps/extension/vite.config.ts`: add Vite alias for `@wendispatch/ai`.
...
## Task 1: Add `@wendispatch/ai` Package
...
```json
"@wendispatch/ai": ["./packages/ai/src"]
```
...
import { db, type AppConfig, type Secret } from '@wendispatch/core';
...
2. Inspect the extension IndexedDB config store.
3. Confirm `ai.rewrite.config` does not contain `apiKey`.
4. Inspect the extension IndexedDB secrets store.
