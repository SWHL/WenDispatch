    ...
    fillAndPublish: function(payload: any): Promise<{ url: string }> {
      ...
      return (async function() {
        try {
          ...
          // 1.5 设置文章封面（掘金编辑页的封面入口由页面动态渲染）
          const coverImage = (payload as any).__coverImage as { base64: string; mimeType?: string } | undefined;
          if (coverImage) {
            try {
              const coverButton = Array.from(document.querySelectorAll('button, [role="button"], span'))
                .find((el: any) => /封面|头图|cover/i.test(String(el.textContent || '').trim())) as HTMLElement | undefined;
              coverButton?.click();
              await sleep(500);
              const fileInput = Array.from(document.querySelectorAll('input[type="file"]'))
                .find((el: any) => /cover|封面|image|图片/i.test(String(el.getAttribute('accept') || '') + String(el.getAttribute('name') || '') + String(el.className || ''))) as HTMLInputElement | undefined
                || document.querySelector('input[type="file"]') as HTMLInputElement | null;
              if (!fileInput) throw new Error('未找到掘金封面上传控件');
              const response = await fetch(coverImage.base64);
              const blob = await response.blob();
              const file = new File([blob], 'cover.png', { type: coverImage.mimeType || blob.type || 'image/png' });
              const transfer = new DataTransfer();
              transfer.items.add(file);
              Object.defineProperty(fileInput, 'files', { configurable: true, value: transfer.files });
              fileInput.dispatchEvent(new Event('change', { bubbles: true }));
              await sleep(1200);
              console.log('[juejin] 封面上传完成');
            } catch (coverError) {
              console.warn('[juejin] 封面上传失败，将继续填充正文:', coverError);
            }
          }
