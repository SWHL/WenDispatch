...
export const useThemeStore = defineStore(`theme`, () => {
  ...
  const updateCodeTheme = () => {
    ...
    // 在扩展环境中，使用内联样式而不是外部 CDN（CSP 限制）
    if (isExtension) {
      // 使用内联样式注入 highlight.js 主题
      const el = document.querySelector(`#hljs-inline`)
      if (!el) {
        // 动态导入本地 CSS 并注入
        import('@/assets/hljs-github-dark.css?inline').then((module) => {
          const style = document.createElement(`style`)
          style.setAttribute(`id`, `hljs-inline`)
          style.textContent = module.default
          document.head.appendChild(style)
        }).catch((err) => {
          console.warn('[updateCodeTheme] Failed to load local hljs theme:', err)
        })
      }
      return
    }
