...
const { themeMode, isEditOnLeft } = storeToRefs(uiStore)
const { setThemeMode, toggleEditOnLeft } = uiStore
  ...
  <div class="theme-customizer space-y-4">
    ...
    <div class="space-y-2">
      ...
      <div class="grid grid-cols-3 justify-items-center gap-2">
        <Button
          title="浅色模式" aria-label="浅色模式" class="w-full" variant="outline" :class="{
            'border-black dark:border-white border-2': themeMode === 'light',
          }" @click="setThemeMode('light')"
        >
          <Sun class="h-4 w-4" />
        </Button>
        <Button
          title="深色模式" aria-label="深色模式" class="w-full" variant="outline" :class="{
            'border-black dark:border-white border-2': themeMode === 'dark',
          }" @click="setThemeMode('dark')"
        >
          <Moon class="h-4 w-4" />
        </Button>
        <Button
          title="跟随系统" aria-label="跟随系统" class="w-full" variant="outline" :class="{
            'border-black dark:border-white border-2': themeMode === 'auto',
